...
if ( ! class_exists( 'CloudFlareAPI' ) ) {
	...
	class CloudFlareAPI {
      ...
      // I need debug info, sorry
			if ( 200 !== $code ) {
        error_log(print_r(new WP_Error( 'response-error', sprintf( __( 'Server response code: %d', 'wp-cloudflare-api' ), $code ) ), true ) );
    ...
    function create_zone( $name, $jump_start = true, $organization = array() ) {
      ...
      if ( $organization != array() && isset( $organization['id'] ) ) {
        ...
        if ( isset( $organization['name'] ) ) {
          $obj['organization']['name'] = $organization['name'];
        }
        if ( isset( $organization['status'] ) ) {
          $obj['organization']['status'] = $organization['status'];
        }
        if ( isset( $organization['permissions'] ) ) { // ex: ["#zones:read","#zones:edit"] array
          $obj['organization']['permissions'] = $organization['permissions'];
        }
      }
